...
for i in "${DOC_RELEVANT[@]}" ; do
    ...
    if [[ $i == *.md ]] ; then
        ...
        printf " --pandoc--> "
        printf "%-35s\n" ${TO/\.md/.html}
        pandoc -s "$FROM" --mathjax --resource-path=.:$XDG_DATA_HOME/pandoc --filter pandoc-crossref --citeproc --filter link_filter.py --standalone --template=easy_template.html -o "${TO/\.md/.html}"
    else
